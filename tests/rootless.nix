...
let
  ...
  # transition subtest: the launcher, its descendants, and every process in
  # the session's cgroup (the hook's daemon and pasta's are reparented). Per
  # process its parent, executable, cgroup, argv and environment (base64,
  # NUL-separated) and each descriptor's target, one line each.
  transitionDump = hostPkgs.writeShellScript "l4-dump" ''
    set -u
    L=$1
    cg=/sys/fs/cgroup/user.slice/user-1000.slice/user@1000.service/app.slice/flong-sessions.service/box/$2
    pids=" $L "
    queue=$L
    while [ -n "$queue" ]; do
      next=
    ...
    for procs in $(find "$cg" -name cgroup.procs); do
      while read -r c; do
        case $pids in *" $c "*) ;; *) pids="$pids$c " ;; esac
      done < "$procs"
    done
    for p in $pids; do
      echo "pid $p"
      echo "ppid $(sed 's/.*) //' /proc/"$p"/stat | cut -d' ' -f2)"
      echo "exe $(readlink /proc/"$p"/exe)"
      echo "cg $(sed -n 's/^0:://p' /proc/"$p"/cgroup)"
      echo "cmdline $(base64 -w0 /proc/"$p"/cmdline)"
      echo "environ $(base64 -w0 /proc/"$p"/environ)"
      for f in /proc/"$p"/fd/*; do echo "fd ''${f##*/} $(readlink "$f")"; done
    done
  '';
  ...
  postStart = ''
    case ''${FLONG_TEST_HOOK:-} in
      ...
      # Asleep until the test kills the sleep whose pid it leaves, then on as
      # a hook that succeeds: the launch held with every child alive.
      hold) sleep infinity >/dev/null 2>&1 & echo $! > "/tmp/hook-hold-$machine"; wait || true ;;
    esac
    {
      id -u
      echo "$machine"
  ...
  testScript = ''
    ...
    def _():
        ...
        out = machine.succeed(as_user("mounts " + shlex.quote(
            ...
            "awk '$5 == \"/run\" { print \"run\", $6 }' /proc/self/mountinfo; "
    ...
    @test("the launcher under test against the C one: bwrap's, pasta's and flong-init's argv, and every child's environment")
    def _():
        # ZIG.md phase 7's L4 (a). Each wrapper used here is copied twice
        # with its launcher= line (module.nix:644) pointing at one of two
        # launchers, and the same warm launches run through both: the
        # reference, flong-launch-c (the C, check only, running the shipped
        # flong-init), and the launcher under test, the shipped one. Each
        # launch is held in its postStart (FLONG_TEST_HOOK=hold) and
        # sampled, then let through to its payload and sampled again: every
        ...
        UNDER_TEST = "${transitionUnderTest}"
        ref = "${launchC}/bin/flong-launch"
        wrapper = lambda w: machine.succeed(f"readlink -f \"$(command -v {w})\"").strip()
        shipped = machine.succeed(f"sed -n 's/^launcher=//p' {wrapper('hooked')}").strip()
        hook = machine.succeed(f"sed -n 's/^post_start=//p' {wrapper('hooked')}").strip()
        assert shipped.startswith("/nix/store/") and shipped.endswith("/bin/flong-launch"), shipped
        assert hook.startswith("/nix/store/") and hook.endswith("/bin/flong-poststart-hooked"), hook

        # The controls that the two sides are the launchers they are meant
        # to be. Two files; the reference names the shipped flong-init,
        # which exists, and says the C's words, as a format.
        assert ref != shipped, ref
        init = shipped.removesuffix("/flong-launch") + "/flong-init"
        machine.succeed(f"test -x {init}")
        machine.succeed(f"grep -qF {init} {ref}")
        machine.succeed(f"grep -qF 'postStart failed (status %d)' {ref}")
        if UNDER_TEST == "c":
            # Until L4: the same C, bit for bit.
            machine.succeed(f"cmp {ref} {shipped}")
        else:
            assert UNDER_TEST == "zig", UNDER_TEST
            machine.fail(f"cmp -s {ref} {shipped}")
            machine.fail(f"grep -qF 'postStart failed (status %d)' {shipped}")

        # The copies: hooked and nethook carry the hook; nested's and
        # project's copies are given hooked's in place of their own (a
        # declaration's post_start= line, module.nix:649). The two copies
        # of each differ in the launcher line alone.
        SIDES = {"ref": ref, "test": shipped}
        machine.succeed("rm -rf /tmp/l4 && mkdir -p /tmp/l4/ref /tmp/l4/test && chmod -R 755 /tmp/l4")
        for w in ("hooked", "nested", "project", "nethook"):
            src = wrapper(w)
            for side, launcher in SIDES.items():
                sed = f"-e 's|^launcher=.*|launcher={launcher}|'"
                if w in ("nested", "project"):
                    sed += f" -e 's|^post_start=.*|post_start={hook}|'"
                machine.succeed(
                    f"sed {sed} {src} > /tmp/l4/{side}/{w} && chmod 755 /tmp/l4/{side}/{w} && "
                    f"test \"$(grep -cx 'launcher={launcher}' /tmp/l4/{side}/{w})\" = 1")
            machine.succeed(f"grep -qx 'post_start=.*/bin/flong-poststart-{'hooked' if w != 'nethook' else w}' /tmp/l4/ref/{w}")
            changed = machine.succeed(f"diff /tmp/l4/ref/{w} /tmp/l4/test/{w} | grep '^[<>]' || true").splitlines()
            assert changed == [f"< launcher={ref}", f"> launcher={shipped}"], (w, changed)
        ...
        def parse(dump):
            ...
            for line in dump.splitlines():
                ...
                if key == "pid":
                    cur = {"fds": {}}
                    procs[val] = cur
                elif key in ("cmdline", "environ"):
                    data = base64.b64decode(val)
                    items = data.split(b"\0")
                    if data.endswith(b"\0"):
                        items = items[:-1]
                    cur[key] = [i.decode(errors="backslashreplace") for i in items]
                elif key == "fd":
                    n, _, target = val.partition(" ")
                    cur["fds"][int(n)] = target
                else:
                    cur[key] = val
            return procs

        # bwrap's options whose value is a descriptor the launcher chose;
        # flong-init's first two arguments are two more (gate and ready).
        FD_OPTS = {"--userns", "--userns2", "--info-fd", "--add-seccomp-fd",
                   "--sync-fd", "--block-fd", "--userns-block-fd", "--json-status-fd"}

        # Every process of the launch, by role, normalised.
        def snapshot(pid, name, launcher, copies):
            procs = parse(machine.succeed(f"${transitionDump} {pid} {name}"))
            assert pid in procs, (pid, procs)

            def exe(p):
                e = procs[p].get("exe", "")
                return "LAUNCHER" if e == launcher else e.rsplit("/", 1)[-1]

            def leaf(p):
                marker = f"/{name}/"
                cg = procs[p].get("cg", "")
                return cg.split(marker, 1)[1] if marker in cg else "caller"
            ...
            role, seen = {}, {}
            for p in sorted(procs, key=int):
                base = f"{leaf(p)}:{chain(p)}"
                role[p] = f"{base}#{seen.get(base, 0)}"
                seen[base] = seen.get(base, 0) + 1
            caller_tty = procs[pid]["fds"].get(0, "")

            def s(x):
                x = x.replace(launcher, "LAUNCHER").replace(copies, "WRAPPERS").replace(name, "MACHINE")
                x = re.sub(r"/proc/(\d+)/", lambda m: f"/proc/[{role.get(m.group(1), 'PID')}]/", x)
                x = re.sub(r"(/proc/\[[^]]*\]/fd/)\d+", r"\1FD", x)
                return re.sub(r"/dev/pts/\d+",
                              lambda m: "/dev/pts/CALLER" if m.group(0) == caller_tty else "/dev/pts/PTY", x)

            def argv(a):
                out = [s(x) for x in a]
                for i in range(1, len(a)):
                    if a[i - 1] in FD_OPTS and a[i].isdigit():
                        out[i] = "FD"
                for i, x in enumerate(a):
                    if x.endswith("/bin/flong-init"):
                        for j in (i + 1, i + 2):
                            if j < len(a) and a[j].isdigit():
                                out[j] = "FD"
                return out
            ...
            for p in procs:
                fds = procs[p]["fds"]
                # 0-2 by number, the rest as a multiset: the launcher may
                # number what it passes as it likes. pasta's own beyond its
                # stdio are its business, and vary: its pool of splice
                # pipes is refilled as it sees fit.
                rest = [] if leaf(p) == "pasta" else sorted(target(t) for n, t in fds.items() if n > 2)
                snap[role[p]] = {
                    "argv": argv(procs[p].get("cmdline", [])),
        ...
        def launch(side, w, env="", relay=False):
            launcher = SIDES[side]
            copies = f"/tmp/l4/{side}"
            machine.succeed("rm -f /tmp/hook-hold-box-* /tmp/poststart-box-* /tmp/daemon-box-* "
                            "/tmp/poststop-box-* /tmp/l4-out /tmp/l4-rc /tmp/l4-stdin")
            cmd = f"{copies}/{w} 'sleep infinity'"
            if relay:
                # The terminal on the launcher's stdin and stdout, so it
                # relays a pty of its own. script's stdin ends only when the
                # test kills the sleep that holds it, after the launcher has
                # gone: script (util-linux 2.42) whose child has exited
                # spins, and never exits, while its stdin is open.
                cmd = (f"script -qfec {shlex.quote(cmd)} /dev/null "
                       "< <(echo $BASHPID > /tmp/l4-stdin; exec sleep infinity)")
            # The unit's per-run variables go, so the two runs' environments
            # can be equal.
            script = (f"unset INVOCATION_ID SYSTEMD_EXEC_PID JOURNAL_STREAM MEMORY_PRESSURE_WATCH MEMORY_PRESSURE_WRITE; export FLONG_TEST_HOOK=hold {env}; "
                      f"{cmd} >/tmp/l4-out 2>&1; echo $? > /tmp/l4-rc")
            machine.succeed(f"{as_user(script)} >/dev/null 2>&1 &")
            hold = machine.wait_until_succeeds(
                "f=$(ls /tmp/hook-hold-box-*) && test -s \"$f\" && echo \"$f\"").strip()
            name = hold.removeprefix("/tmp/hook-hold-")
            pid = launcher_pid(name)
            # The run's launcher is the side's.
            exe = machine.succeed(f"readlink /proc/{pid}/exe").strip()
            assert exe == launcher, (side, w, exe)
            held = snapshot(pid, name, launcher, copies)
            machine.succeed(f"kill $(cat {hold})")
            machine.wait_until_succeeds(f"session-of box | grep -qx {name}")
            running = snapshot(pid, name, launcher, copies)
            machine.succeed(f"kill -TERM {pid}")
            if relay:
                machine.wait_until_succeeds(f"test ! -e /proc/{pid}")
                machine.succeed("kill $(cat /tmp/l4-stdin)")
            machine.wait_until_succeeds("test -s /tmp/l4-rc")
            rc = machine.succeed("cat /tmp/l4-rc").strip()
            # pasta-wait: the launcher is gone only once pasta has let the
            # forwarded port go.
            port = machine.succeed("ss -Hltn 'sport = :18200'")
            out = []
            for l in machine.succeed("cat /tmp/l4-out").replace("\r", "").splitlines():
                f = l.split()
                # A trace line's clock differs run to run; its stage does not.
                out.append("T " + " ".join(f[2:]) if f[:1] == ["T"] else l.replace(name, "MACHINE"))
            # Nor does which of two processes writes first once the gate is
            # open: the launcher traces gate-open after its write to the gate
            # (flong-launch.c:695-701), flong-init payload-exec once it has
            # read it (src/init.zig:255), so under load payload-exec can
            # come first. The launcher's own order is compared.
            for i in range(len(out) - 1):
                if out[i:i + 2] == ["T payload-exec", "T gate-open"]:
                    out[i:i + 2] = ["T gate-open", "T payload-exec"]
            machine.succeed(f"test ! -e {CG}/box/{name}")
            return {"held": held, "running": running, "rc": rc, "port": port, "out": out}

        # What differs, by role and field, as a unified diff of the two
        # sides' lines.
        def diff(what, a, b):
            if a == b:
                return
            ...
            for r in sorted(set(a) | set(b)):
                ...
                for field in sorted(set(ra) | set(rb)):
                    ...
                    if fa != fb:
                        la = [repr(x) for x in fa] if isinstance(fa, list) else [repr(fa)]
                        lb = [repr(x) for x in fb] if isinstance(fb, list) else [repr(fb)]
                        lines.append(f"{r} {field}:")
                        lines += difflib.unified_diff(la, lb, "ref", "test", n=1, lineterm="")
            raise AssertionError("\n".join(lines))

        def has(snap, ending):
            return [r for r in snap if r.split("#")[0].endswith(ending)]

        # Warm, both: box's cache, and the project policy's compiled filter.
        POLICY = shlex.quote("allow ptrace")
        machine.succeed(as_user("hooked true"))
        machine.succeed(as_user(f"FLONG_TEST_POLICY={POLICY} project true"))

        VARIANTS = {
            "plain": ("hooked", "", False),
            "relay": ("hooked", "", True),
            "nested": ("nested", "", False),
            "project": ("project", f"FLONG_TEST_POLICY={POLICY}", False),
            # hostPorts and forwardPorts, so pasta-wait, and resolv.conf's
            # keep-fd; traced, so the wait says it happened.
            "network": ("nethook", "FLONG_TRACE=1", False),
            "trace": ("hooked", "FLONG_TRACE=1", False),
        }
        runs = {}
        for v, (w, env, relay) in VARIANTS.items():
            ...
            assert r["rc"] == t["rc"] == "143", (v, r["rc"], t["rc"])
            assert r["out"] == t["out"], (v, r["out"], t["out"])
            assert r["port"] == t["port"] == "", (v, r["port"], t["port"])

            # The controls that the samples hold what they are said to:
            # bwrap, flong-init at the gate and the hook while held; the
            # payload, and the hook's daemon, once through.
            held, running = r["held"], r["running"]
            for ending in (":LAUNCHER>bwrap", ">flong-init"):
                assert has(held, ending), (v, ending, sorted(held))
            hooks = has(held, ":LAUNCHER>bash")
            assert len(hooks) == 1 and hooks[0].startswith("hooks:"), (v, sorted(held))
            e = held[hooks[0]]["env"]
            leader = [x for x in e if x.startswith("leader=")]
            assert len(leader) == 1 and re.fullmatch(r"leader=\[sandbox:[^]]*\]", leader[0]), (v, leader)
            for want in ("userns=/proc/[caller:LAUNCHER#0]/fd/FD", "netns=/proc/[caller:LAUNCHER#0]/fd/FD",
                         "machine=MACHINE"):
                assert want in e, (v, want, e)
            bwrap = held[has(held, ":LAUNCHER>bwrap")[0]]["argv"]
            for opt in ("--userns", "--userns2", "--info-fd"):
                assert bwrap[bwrap.index(opt) + 1] == "FD", (v, opt, bwrap)
            assert [r for r in running if r.startswith("sandbox:") and running[r]["argv"] == ["sleep", "infinity"]], (v, sorted(running))
            assert [r for r in running if r.startswith("hooks:")], (v, sorted(running))
            pasta = [r for r in running if r.startswith("pasta:")]
            watchdog = has(running, "caller:LAUNCHER>LAUNCHER")
            assert bool(pasta) == (v == "network"), (v, sorted(running))
            assert bool(watchdog) == (v == "relay"), (v, sorted(running))
            if v == "network":
                p = running[pasta[0]]["argv"]
                for opt, want in (("--userns", "/proc/[caller:LAUNCHER#0]/fd/FD"), ("--pid", "/proc/[caller:LAUNCHER#0]/fd/FD"),
                                  ("-T", "18123"), ("-t", "18200:18201")):
                    assert p[p.index(opt) + 1] == want, (opt, want, p)
                assert re.fullmatch(r"/proc/\[sandbox:[^]]*\]/ns/net", p[p.index("--netns") + 1]), p
                assert "--ro-bind-data" in bwrap and "/etc/resolv.conf" in bwrap, bwrap
            init = held[has(held, ">flong-init")[0]]["argv"]
            assert ("ctty" in init) == (v == "relay") and ("trace" in init) == (v in ("trace", "network")), (v, init)
            assert ("--assert-userns-disabled" in bwrap) == (v != "nested"), (v, bwrap)
            if v == "trace":
                assert "T launcher-start" in r["out"] and "T gate-open" in r["out"], r["out"]
            elif v == "network":
                # pasta-wait: teardown waited for pasta's leaf to empty
                # (flong-launch.c:817-825), after postStop.
                assert r["out"].index("T pasta-gone") > r["out"].index("T poststop-done"), r["out"]
            elif v == "relay":
                # The relay's OSC 666 marks, naming the container and
                # clearing it (flong-tty.c:119-121, 159-160), which the
                # caller's terminal receives.
        ...
        for v in ("relay", "nested", "project", "network", "trace"):
            assert runs[v]["ref"]["held"] != runs["plain"]["ref"]["held"], v
        machine.succeed("rm -rf /tmp/l4 /tmp/l4-out /tmp/l4-rc /tmp/l4-stdin")
