...
#
# `aarch64` is the archive built for aarch64 and its clash check, which
# native.nix's cross-aarch64 builds (P6's piece of it). P5 stays until phase
# 4's clash check on the real link replaces it (ZIG.md, "Phase 2").
#
...
let
  ...
  # The same archive for aarch64-linux-none (P6, spike/proofs/p6, now in
  # native.nix's cross-aarch64): proof_main its only global definition, and
  # nothing undefined that glibc does not define. The aarch64 C link is out
  # of scope: the flake has no cross C toolchain (ZIG.md, "The mount-helper
  # shim"). What it found is in $out/facts.
  aarch64 = zigSet {
    pname = "p5-aarch64";
    root = ./.;
    files = [ ./src ];
    ...
    extra = ''
      ...
      rmdir $out/lib
      readelf -h $a | grep -q 'Machine: *AArch64' || { readelf -h $a; echo "p5: libp5.a is not aarch64"; exit 1; }
      echo "libp5.a: aarch64, $(stat -c %s $a) bytes" | tee -a $out/facts

      # The clash check's archive half, as the x86_64 build runs it.
      globals=$(nm -g --defined-only $a | awk 'NF == 3 { print $2, $3 }')
      echo "libp5.a global definitions: $globals" | tee -a $out/facts
      [ "$globals" = "T proof_main" ] || { echo "p5: the aarch64 archive defines more than proof_main"; exit 1; }
      ...
      for s in $undef; do
        case $s in
          ...
          *) echo "p5: the aarch64 libp5.a needs $s, which the aarch64 C link may not resolve"; exit 1 ;;
        esac
      done
    '';
  };
