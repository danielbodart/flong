...
| the Zig fixtures against the C (phase 6 a) | `checks.fixtures-transition`, the C built in the check, 111 comparisons (stdout by `cmp`, stderr, status), all equal: `bpfdump eval` over the 14 tier variants alone and stacked with audit, tty and nsmask (swept with parity's and strict's constants), audit, tty, nsmask, the 23 golden `.bpf` files and golden-update's call; 21 refusals, the empty name, a procfs file, and an every-opcode program, one block per opcode whose result shows (8,784 lines; it catches 30 of 30 one-opcode plants and 5 boundary ones, the old one 1); `syscall-probe` in the sandbox, and its calls under `strace` in raw values (setns's `-1` a 32-bit 0xffffffff, fork's clone flags glibc's 0x1200011); 27 `ioctl-probe` runs (22 spellings of a request, a positive return (`NS_GET_NSTYPE`, "0" as the C), stdin closed, usage); `swapper`'s usage, refusals and a live run of each (three round trips within 60 s). Controls: `pair` refuses one binary on both sides; the sides are different files, the C's needing libc, the three Zig's static; `same` sees each kind of difference; plants caught: a name column of 23 (56 differences), `0b` not read (one). Parity VM: both dump one payload's 4 filters byte for byte, evaluate both tiers' live stacks identically (15,092 lines each), and give the same 39 probe lines, stderr empty, under each tier. Rootless: the tty filter's requests equal under plain and nested and on a terminal; the swap race 20+20 with each swapper, 0 escapes (the C 23 refused, 17 contained; the Zig 17, 23 in the commit's run). A review's differential fuzz: 996 `bpfdump eval` runs of 1-3 random kernel-valid programs, 0 differences; `ERRNO(NAME)` equal for all 65,536 data values, syscall names for 3,072 (arch, nr). The walker (`checks.native`), each swapper: naive control escaped 104-108 (Zig) and 56-96 (C) of 200 (the Zig 55 in the commit's run), the walk 0 of 400; each swapper killed with 143 while swapping, naive contained > 0. `test-libc`: ioctl-probe's `strtoul0` equal to glibc's `__isoc23_strtoul` and bpfdump's `atoi` to `atoi` (every string of up to 3 over 17 characters, 24 edges, 20,000 random); scmp's `seccomp_syscall_resolve_num_arch` and `free` against translate-c | this host, 2026-09-24, phase 6 (a) |
| the fixtures set (phase 6 a) | `nix build --rebuild` 8.5-9.3 s, bit-identical; bpfdump 43,872 bytes (needs libseccomp and libc), syscall-probe 22,104, swapper 20,584, ioctl-probe 19,080, static, stripped, PT_GNU_STACK size 0; closure 38,019,032 bytes; aarch64: the three built, bpfdump compiled unlinked. `scmp.zig` gained bpfdump's two externs, which moved seccomp's store path (quirk 36). Local `nix flake check -L`, alone: green, 290.0 s (rootless 110.9 s; the rest cached from the reviews' builds of the same tree: basic 110.0 s, native 17.9 s, parity 22.0 s, the last after its stderr edit); `--no-build --all-systems` 281 s. CI before it (d83510f, run 35947311755): `nix flake check` 9 min 25 s | this host, 2026-09-24; CI |
| the Zig spec against the C (phase 7 L1) | `golden`'s spec cases (163, two new: a closure through a file, a closure resolving to `/nix/store`) equal against `flong-launch` and `spec-probe`; `spec-accepted`'s one case exits 0 under the probe and 125 under the C, telling the sides apart. `native-test`: `bwrapArgv`'s golden argv for plain, relay, nestedSandbox, filters with a project policy, keep-fds and trace; specs drawn from a model parse back to it; 45+ single-rule mutations, each refused with its message, fixed and as a property. `paths.txt`, 44 cases (7 meant to differ), held by `spec.clean`/`mount` (`spec-paths`) and by `module.nix`'s `hostAssertions`. A review's differential fuzz, `spec-probe` against the C: 8,000 random mutations (668 accepted), about 15,000 field substitutions (number edges, NAME_MAX and PATH_MAX, the 1024-byte cut, non-UTF-8, store symlinks), 24,000 idmap/user/group combinations, argc 0 and 1: 0 differences in stderr or status. Plants, each caught: a refusal reordered (golden cross-order-6), three checks skipped and `--perms` anywhere (the mutation test and property), `--unshare-cgroup` or `--new-session` dropped (the golden argv), a message byte (golden, native-test), a `paths.txt` verdict (both sides), the closure prefix without its slash and the probe running the C (each only after its new case). Kept: the closure's O_PATH open needs a free descriptor and `/proc` (quirk 21), where glibc's realpath needs neither; the probe's root refusal untested until L4 | this host, 2026-09-24, phase 7 L1 and its reviews |
| phase 7 L1's runs | `native-lint` 5 s, `native-analyze` 79 s, `cross-aarch64` 19 s, `integration` 22 s, `golden` about 40 s, the spec tests alone (Debug) about 72 s. Local `nix flake check -L`, alone: green, 391 s (test scripts native 25.0 s, parity 27.8 s, basic 127.2 s, rootless 114.3 s); `--no-build --all-systems` 282 s. The launcher's store path moved (its fileset holds `src/`), seccomp's did not. CI before it (5f1f08e, run 35958022199): `nix flake check` 12 min 48 s | this host, 2026-09-24; CI |
| namespaces, cgroups, records, passwd in Zig (phase 7 L2) | `checks.native` (`tests/launch-native.nix`, through `launchdriver`): U1 and U2's maps read back through `/run/wrappers/bin/newuidmap`, refusals leaving nothing; the U2 abort (Aborted, no pipes, no children); U2's child refused an EINVAL limit (`nested_userns=4294967296`: one message, no second for either 125, `rc=1`, nothing left); U2's helper failing its maps (`u2-maps`, exact output, nothing left); the nsdelegate check, holder and a session; a record created, closed and swept by the Zig sweeper; passwd against `getent` and the `uid N` fallback. `test-libc`: the mountinfo reader against `cg_check_nsdelegate`, 13-file corpus. `launch-test`: the writer against `tests/golden/records/`. Plants, each caught: maps judged before both reaped; "m" before the maps (only by `u2-maps`, rc=137; before it, u2-abort caught it now and then by a race); pid before "n" (the EINVAL case); the helper reaped before its pipes close (rc=137); `rep`, `up` or `hold` left open; the limit write skipped; the EEXIST loop counted to 1; `leader=` by pwrite at 0; no flock; remove without unlink; passwd lookup null, its uid text, last match winning; 125 reported by U2's helper. Held by review only (plants pass): U1/U2 opened after the hold/release close; an EEXIST count of 2 or more; close before unlink, link before write; `.wait` for `.kill` on U1's abort; the map programs one after the other. `u2Helper` exits with `u2Steps`' answer: one noreturn function made `native-analyze` report a leak at the fork of U2's child | this host, 2026-09-24, phase 7 L2 and its reviews |
| phase 7 L2's runs | `native-test-release` + `native-analyze` + `native-lint` 102.6 s; `native-test-debug` + `cross-aarch64` + `integration` + `golden` + `native` 81.4 s (L2 subtests 0.14-0.82 s; test script 37.7 s); `checks.native` alone 49.5-49.6 s; the ten touched checks (those, `launcher`, `seccomp`) 120.9 s, partly cached. seccomp's store path moved (it imports `sys`, `fd`): 144,784 bytes either way, the bytes differ; the launcher's moved | this host, 2026-09-24 |
| the Zig terminal (phase 7 L3) | `src/tty.zig` ports `flong-tty.c`, run only by its tests and `flong-tty` (`tests/zig/ttydriver.zig`: the launcher's calls in its order, the program standing in for bwrap and the payload). `checks.native`: L0's `rootless.nix` lines again (keys 7, escape 137, stderr kept, resize, the watchdog's `guard=0:tty 1:tty 2:tty pidfd pipe` and restore, SIGCONT, hang-up 129); the reopen control; a root-owned pty under `setpriv` giving `tty.out: fd 1` with every line; EIO sleeping in poll and SIGTERM still 143; the drain after TCOOFF (`before=0`, 2000 lines, 3); stdin at EOF then SIGWINCH, 5 not 125. `cfmakeraw` against glibc's (translate-c): 17 constants, each flag bit, all bits, 20,000 random structs; `abi.zig` checks termios, winsize and 31 constants on x86_64 and aarch64. The ^] property: 10,000 runs fixed seed, 10,000 random, against a model with its own literals, counters requiring over 1,000 three-at-one-instant cases and completions. `finish` twice on a real relay: modes back exactly, one mark, one clear, every field cleared. SIGTTOU scopes: ignore and `defer` restore, `makeRaw` not ignoring. Plants, each caught: EIO leaving the master open, no drain, `resize` ignoring the null master, `>=` for the window, a key not resetting the count, 4 presses, SIGWINCH ignored, SIGCONT not re-raw, the watchdog not restoring, the fd-1 fallback missing (two ways), `finish` not idempotent or not restoring (only after the new test), `restoreModes` without its `defer`, `cfmakeraw` without IXON; 4 presses passed the property until the model stopped sharing the code's constants. Untested (as in L0): the passthrough watchdog and handing the foreground back, for L4/L5. On kernel 6.18 a master write after the last slave closes succeeds, so the C's EIO comment for input writes (`flong-tty.c:467-472`) is not asserted | this host, 2026-09-24, phase 7 L3 and its reviews |
| phase 7 L3's runs | `flong-tty` ReleaseSafe, stripped, x86_64: 75,064 bytes. Test scripts: native 25.9 s (the tty subtest 7.1 s, scenarios 0.3-1.1 s, 3 green runs), rootless-b's terminal subtest 5.4 s (check 82 s); `native-analyze` 76 s; basic-a 55.8 s, basic-b 54.2 s, rootless-a 38.3 s, parity 21.9 s (the four together 107 s). `sys.zig` and `fd.zig` are seccomp sources, so seccomp's store path moved, and the launcher's | this host, 2026-09-24 |
| the Zig flong-launch (phase 7 L4) | `src/launch.zig` composes `src/launch/*`, spec, ns, cgroup, record, tty and mount (a fork body, no shim); the launcher set ships it, static, no INTERP, no symbol table, PT_GNU_STACK size 0, `disallowedReferences` holding: 446,448 bytes (the C 234,976; flong-init 40,312 and flong-sweeper 142,272 unchanged), the closure 44,256,152 bytes (44,045,288), aarch64 built static by `cross-aarch64`; the launcher derivation, `--rebuild`, 19.6 s (the C's 16.8 s), bit-identical. The rootless transition subtest, Zig against `flong-launch-c`, over its six variants: bwrap's, pasta's and flong-init's argv, every child's environment and descriptors, and the launcher's own descriptors (a multiset; one allowed extra, the watchdog's pidfd in relay after the gate, quirk 9), stdout, stderr and status all equal; a traced launch refused by postStart equal too (its refusal, `mounts-done`, `poststop-done`, `released`, 125). Controls: the two binaries differ, the shipped one has none of the C's format strings. Plants in rootless-a, each red: ready_r kept past checkpoint 3 (green before the launcher's own table was compared), info_w kept past checkpoint 2, the gate before postStart, pasta's `--quiet` dropped, pasta-wait before postStop, a cgroup descriptor leaked into bwrap, both sides the same launcher. A review's `strace -f` of both launchers, 10 variants twice each (40 launches, equal statuses): with quirks 9, 21 and 23's mechanisms and glibc's own steps removed, the launcher and each child it forks or spawns make the same calls in the same order, apart from one close in U2's helper before its exit. Review fixes: `released` traced only with the cgroup (`flong-launch.c:836-837`), the sandbox leaf unwrapped, `run`'s step numbers DESIGN's. `golden`'s 164 spec cases (one new, `state-missing`, the valid spec stopping at the state directory, recorded from the C) run against the shipped `flong-launch`; `spec-probe` and `spec-accepted` gone. `nix run .#gate`, alone, green three times (3 min 1 s, 3 min 24 s, 3 min 2 s, partly cached; rootless-a's test script 60.6-69.6 s, its transition subtest 28.8-31.3 s); `gate-aarch64` evaluates (34 s). The previous trunk CI run: 14 min 53 s for af2bfc2 (3 min 39 s for 6acfa9b, docs only). `nix build .#bench`, medians of 20 (p10-p90) over three runs, ms, origin/trunk 6acfa9b (C) then this tree (Zig): no network 33.6 (30.3-35.6), 33.1 (31.0-34.1), 32.5 (27.6-34.5) / 34.0 (31.4-35.7), 32.9 (29.1-34.8), 30.8 (28.8-33.6); pasta + nft hook 60.1 (56.8-65.7), 53.1 (43.2-58.5), 55.2 (44.3-63.3) / 59.3 (57.4-65.7), 52.7 (49.8-59.5), 52.5 (50.3-58.5); forwarded port 76.9 (70.1-85.8), 72.0 (60.5-80.9), 79.0 (70.7-85.9) / 73.9 (68.1-80.8), 72.2 (67.4-81.1), 71.4 (64.8-87.1); cold 358.2 (320.9-383.8), 356.5 (328.0-395.5), 355.7 (303.9-379.1) / 382.0 (360.9-401.7), 374.7 (343.0-386.1), 378.1 (344.8-394.0). The earlier pair (af2bfc2 against the Zig) had cold the other way round, 373.5, 368.2, 379.4 / 352.9, 374.0, 368.8: reported, not gated | this host, 2026-09-24, phase 7 L4 and its reviews |
| the C launcher deleted (phase 7 L5) | `git ls-files '*.c'` lists nothing: `launcher/*.c` and `*.h`, `src/hybrid/`, `fd.adoptForeign` (its tests open their own namespaces with `fd.openUserns` and `fd.openNetns`), `mountlib`, the clash check and shim run, `flong-launch-c`, the rootless transition subtest and its `hold` hook, `tests/zig/mount.h`, `libc_mount.zig`, `libc_record.zig`, `record_c.c`, `mountinfo_c.c` are gone; fdlint's `adopt-foreign` and `extern` allow-lists lose `mount_c.zig` (good.zig linted as `scmp.zig`). Kept, with a reference still: `libc_launch.zig`'s `sys.O_TMPFILE` against glibc, now through translate-c of `tests/zig/fcntl.h`; the mountinfo differential's 28 fixed cases and its long line, each with the C's verdict, are a `cgroup.zig` unit test (a flipped verdict planted: red). `nix run .#gate`, alone, green, 165 s (partly cached; test scripts rootless-a 38.1 s, 17 of 33 subtests, rootless-b 40.1 s, basic-a 62.9 s, basic-b 56.6 s, native 39.8 s, parity 26.4 s); `gate-aarch64` evaluates (34 s) | this host, 2026-09-24, phase 7 L5 |
...
- **`tests/integration.nix`**, from phase 0, never an output: `zig build
  ...
  walker, ns and pty drivers; phase 7's `spec-probe` until L4, and its
  `flong-launch-c` from L4 to L5) for `checks.native`, `golden` and the
  rootless transition subtest. Phase 0's is per-proof derivations instead: it finds each
